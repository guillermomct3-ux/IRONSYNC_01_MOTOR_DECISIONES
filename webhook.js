...
async function cargarEquipos() {
  try {
    ...
    if (error) { console.error("Error cargando equipos:", error.message); return; }
    EQUIPO_ALIASES = data
      .map(e => e.nombre.replace(/\s+/g, "").toUpperCase())
      .sort((a, b) => b.length - a.length);
    console.log("Equipos cargados:", EQUIPO_ALIASES);
  } catch (err) {
...
function separarEquipoPegado(body) {
  ...
  for (const alias of EQUIPO_ALIASES) {
    if (pegado.startsWith(alias) && pegado.length > alias.length) {
      ...
      if (/^\d+$/.test(contador)) return trigger + " " + alias + " " + contador + resto;
    }
  }
  return body;
}
